...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction { case Json.Num(n) => if (n.isValidInt) Some(n.toInt) else None }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(ar) => {
        val list = ar.map(v => v.decodeAs[A])

        if (list.contains(None)) None
        else Some(list.map(_.get))
      }
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{
      case Json.Obj(o) => o.get(name).flatMap(_.decodeAs[A])
    }
